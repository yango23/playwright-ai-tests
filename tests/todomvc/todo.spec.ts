// tests/todomvc/todo.spec.ts
import { expect } from '@playwright/test';
import { uiTest as test } from '../UI/fixtures';

// Legacy compat shim: this file mirrors the moved tests in `tests/UI/todomvc`
// to preserve history and any external tooling that expects the old path.
...
test('[ui][todo][regression] editing todo to empty string removes the task', async ({ todoPage }) => {
  ...
  // Editing to an empty string should remove the task (classic TodoMVC behavior)
